...
- Активный опрос `GET /api/oauth/usage` с OAuth-токеном профиля — без hook и без действий пользователя.
- Все лимиты из `limits[]`, включая привязанные к моделям.
- Запасной источник: `cachedUsageUtilization` из `.claude.json`, когда токен истёк.
- Почта и тариф из `.claude.json`.
- Необязательный status-line hook (`--hook`) с backup `settings.json`.
...
- Панель показывает пятичасовой лимит, при его отсутствии — недельный, у провайдеров без лимитов — баланс.
- Кнопки ручного обновления нет: этим занимается сервис.
...
- Живой DeepSeek-ключ: баланс в USD.

## Что обязательно требует тестирования

- Popup расширения при разных наборах аккаунтов, file monitor, многократные enable/disable, длинные строки. Само расширение после перезахода в сессию включается и держится в состоянии ACTIVE без ошибок в журнале.
- DeepSeek: `depleted` и 401/403 на живом ключе (успешный ответ уже проверен).
...
- OAuth-токен Claude живёт около 8 часов и обновляется только самим Claude Code; дольше этого срока виджет показывает кеш Claude Code с пометкой возраста.
- Endpoint `/api/oauth/usage` не документирован публично: его формат может измениться.
