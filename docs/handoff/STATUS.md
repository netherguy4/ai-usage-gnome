...
- 110 unit-тестов на фикстурах реальных ответов провайдеров.
...
- Не чаще одного запроса в 5 минут, `429` наращивает паузу до часа; момент следующей попытки переживает перезапуск сервиса.
- Все лимиты из `limits[]`, включая привязанные к моделям.
- Собственный кеш последнего удачного ответа в runtime-каталоге, права `0600`.
- Запасной источник: `cachedUsageUtilization` из `.claude.json`, когда токен истёк; из двух источников берётся более свежий.
- Лимит с прошедшим `resets_at` в запасных данных не показывается.
...
- `cargo fmt --check`, `clippy -D warnings`, `cargo test` (110 тестов), release build — все зелёные.
...
- Popup расширения при 0 и 1 аккаунте, многократные enable/disable, длинные строки. Панель и меню с тремя настоящими аккаунтами отрисованы в живой сессии: логотипы, тарифы, почты, все лимиты, разделители и баланс на месте.
...
- Production-ready статус: код и локальная приёмка готовы, но релиз не публиковался.
...
- Он же ограничивает частоту и отвечает `429` без полезного `Retry-After` (измерено: приходит `Retry-After: 0`). Выбранные 5 минут и отход до часа подобраны эмпирически на живом аккаунте, а не по документации; бюджет делится с самим Claude Code, поэтому периодический отказ нормален.
