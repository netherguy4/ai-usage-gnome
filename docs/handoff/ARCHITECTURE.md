...
- panel label показывает значок каждого провайдера и его пятичасовой лимит, а при отсутствии такого — недельный. Минимум по всем окнам не годится: исчерпанный лимит отдельной модели показывал бы 0%, хотя работать ещё можно;
